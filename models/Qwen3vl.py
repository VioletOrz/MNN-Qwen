...
def clean_llm_output(text, repeat_char_threshold=3, sentence_similarity=0.9):
    ...
    # ---------- Step 1. 去掉连续重复字符 ----------
    text = re.sub(r'(.)\1{' + str(repeat_char_threshold - 1) + r',}', r'\1', text)
    ...
    for s in sentences:
        if not cleaned:
            cleaned.append(s)
            continue
        sim = difflib.SequenceMatcher(None, s, cleaned[-1]).ratio()
        if sim < sentence_similarity:
            cleaned.append(s)
    ...
    # ---------- Step 7. 若全为无意义字符则返回空 ----------
    if not re.search(r'[\u4e00-\u9fa5a-zA-Z0-9]', result):
        result = ""
        ...
        if len(self.history['text']) > windows_size:
            if self.history['text'][0]['role'] == 'system':
                if self.history_window == 0:
                    new_history = [self.history['text'][0]]
                else:
                ...
                if self.history_window == 0:
                    ...
                    new_history = self.history['text'][-self.history_window:]
